function initButtons(){
		$(".dismiss").bind("click", function(){
			...
			centerOverlay.noMoreResize();
								   
			window.setTimeout(function(){
				$(".overlay").hide();
				$(".blocker").hide();
				var _videoHolder = document.getElementById("videoHolder");
				_videoHolder.parentNode.removeChild(_videoHolder);
			},500)
		...
		$(".volume").each(function(index){
			$(this).attr("value", volumeArray[index] * 100).change(function(i){
				...
				finalGainStageArray[index].gain.value = channelGain;
				muteArray[index] = 1;
		...
		$(".overdriveFeedback").each(function(index){
				$(this).change(function(){
					that = $(this);
					reverbMix = that.val();
					reverbGainArray[showPattern].gain.value = reverbMix;
					cleanGainArray[showPattern].gain.value = 1 - reverbMix;
					forString = ( reverbMix * 100 ).toFixed(0)
					$("#overdriveFeedback").html("Amount: " + forString + "%")
					overdriveFeedbackArray[showPattern] = reverbMix;
					console.log(reverbMix)

				})
		})
		$(".dump").bind("click", dump)
		...
		function innerDump(){
			if(fxOnArray[showPattern]){
				...
				$(this).removeClass("fx").addClass("activeFx")
				finalGainStageArray[showPattern].disconnect();
				finalGainStageArray[showPattern].connect(context.destination)
		...
		function dump(){
			...
			finalGainStageArray[rowIndex].connect(context.destination)
...
function handlePause(){
	console.log(delete pageButtons.rowA)
	delete pageButtons;
	isPlaying = 0;
	$(this).html("")
	...
	for(var i = 0; i < 20; i++){
		if(i < 12){
			currentButton = pageButtons.getButton(i,lastStepArray[showPattern] + 1)
			if(!allRhythms[showPattern][i][lastStepArray[showPattern] + 1]){
				$(currentButton).removeClass("active")
			}
		}
		else if(!isMiniLooping[i - 12]){
			currentButton = pageButtons.getButton(i,lastStepArray[i - 12] + 1)
			$(currentButton).removeClass("active")
		}
		else{
			currentButton = pageButtons.getButton(i,lastStepArray[i - 12] + 1)
...
function muteCall(){
	if(muteArray[$(this).prop("rowIndex")]){
		...
		finalGainStageArray[$(this).prop("rowIndex")].gain.value = 0;
		$(this).removeClass("mute").addClass("muted")
	}
	else{
		muteArray[$(this).prop("rowIndex")] = 1;
		finalGainStageArray[$(this).prop("rowIndex")].gain.value = volumeArray[$(this).prop("rowIndex")];
...
function stepToggle(){
	...
	for(var i = 0; i < a7Array.length; i++){
